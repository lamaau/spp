<div {{ $attributes }}>
    <template x-if="defaultTableComponent">
    ...
    <div class="px-4 py-4 -mx-4 overflow-x-auto sm:-mx-8 sm:px-8">
        <div class="relative inline-block min-w-full overflow-hidden rounded shadow">
            <table class="min-w-full leading-normal">
                            ...
                            <th
                                ...
                                <template x-if="!column.visibility || column.visibility === 'visible'">
                                    <div class="flex flex-wrap justify-left"
                                        x-bind:style="column.width ? `width: ${column.width}` : 'width: 5rem'">
                ...
                <tbody class="relative"
                    x-bind:class="{'bg-gray-300 opacity-50' : tableLoading, 'bg-white' : !(tableLoading)}">
                    <div x-show="tableLoading" class="absolute z-50 p-4 text-white bg-gray-600 rounded-md"
                    ...
                    <template x-if="!tableLoading && !items.length">
                        ...
                        <span class="relative z-0 inline-flex shadow-sm">
                            <span aria-disabled="true" aria-label="{{ __('pagination.previous') }}">
                                <span x-on:click="changePage(pagination.current_page-1)"
                                    x-bind:class="{ 'cursor-not-allowed' : pagination.current_page <= 1 }"
                            ...
                            <template x-for="item in pages">
                                <span aria-current="page">
                                    <span x-on:click="changePage(item)"
                                        x-bind:class="{'bg-indigo-600 text-white cursor-not-allowed' : active == item, 'text-gray-500' : active !== item}"
                                        class="relative inline-flex items-center px-4 py-2 text-sm font-medium leading-5 bg-white border border-gray-300 cursor-pointer "
                                        x-text="
                                        item"></span>
                                </span>
                            </template>
                            <span aria-disabled="true" aria-label="{{ __('pagination.next') }}">
                                <span x-on:click="changePage(pagination.current_page+1)"
                                    x-bind:class="{ 'cursor-not-allowed' : pagination.current_page >= pagination.last_page }"
